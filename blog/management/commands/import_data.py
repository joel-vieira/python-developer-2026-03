...
DATA_INFORMATION = [
    {"model": Author, "directory_path": "./data/authors/"},
    {"model": Post, "directory_path": "./data/posts/"},
]
...
class Command(BaseCommand):
    ...
    def handle(self, *args: list, **options: list) -> None:
        for data_information in DATA_INFORMATION:
            if not os.path.isdir(data_information["directory_path"]):
                self.stderr.write(
                    f"Could't find directory: {data_information['directory_path']}"
                )
                return
    ...
    def fetch_data_from_json(self, directory_path: str) -> list:
        data = []
    ...
    def upload_to_database(self, model: Author | Post, data: list) -> None:
        with transaction.atomic():
            for item in data:
                ...
                if created:
                    self.stdout.write(
                        f"Successfully created {model.__name__}: {obj.id}"
                    )
                else:
                    self.stdout.write(
                        f"Warning: {model.__name__} {obj.id} already exists!"
                    )
